...
(function () {
  ...
  function setupFullAddressTruncUppercase() {
    ...
    if (fullAddressTruncElement) {
      ...
      fullAddressTruncElement.removeEventListener('input', handleUppercaseInput);
      ...
      console.log('[PENCIL] Set up uppercase input handler for full_Address_Trunc');
    }
  }

  // Handle input events to convert to uppercase
  ...
  function populateConstructedFields(data) {
    ...
    // Populate full_Address_Trunc field (fragment format) - ENSURE UPPERCASE
    const fullAddressTruncElement = document.getElementById('full_Address_Trunc');
    if (fullAddressTruncElement) {
      fullAddressTruncElement.value = constructedFields.full_Address_Trunc.toUpperCase();
      console.log(`[PENCIL] Populated full_Address_Trunc: ${constructedFields.full_Address_Trunc.toUpperCase()}`);
    }

    // Keep backward compatibility with old full_address field - ENSURE UPPERCASE
    const fullAddressElement = document.getElementById('full_address');
    if (fullAddressElement) {
      fullAddressElement.value = constructedFields.full_Address_Trunc.toUpperCase();
      console.log(`[PENCIL] Populated full_address (legacy): ${constructedFields.full_Address_Trunc.toUpperCase()}`);
    }
  }
  ...
  function getOrCreateConstructedFields(customerInfo) {
    ...
    // Keep original case for name_Address_Combo
    ...
    // Only city is uppercase for name_Address_Combo
    let addressPart = '';
    if (city) {
      addressPart = city.toUpperCase();
      if (street) {
        addressPart += `, ${street}`;
        if (flatNumber) {
    ...
    // Build full_Address_Trunc: "17 Main Street EDINBURGH" (no postcode) - ENSURE UPPERCASE
    ...
    const full_Address_Trunc = addressTruncParts.join(' ').toUpperCase();
  ...
  function repopulateFieldsAfterFragmentLoad() {
    ...
    // Set up uppercase input handler for full_Address_Trunc after fragment load
    setTimeout(() => {
  ...
  function initializePencilHandler() {
    ...
    // Set up uppercase input handler for full_Address_Trunc
    setupFullAddressTruncUppercase();
  ...
  window.PENCIL_DEBUG = {
    ...
    setupFullAddressTruncUppercase: setupFullAddressTruncUppercase
  };
