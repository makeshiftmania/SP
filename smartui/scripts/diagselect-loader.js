...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateDeviceReadOptions() {
    ...
    // Attach click event to close dropdown and set value
    blankOption.addEventListener('click', function(event) {
    ...
    if (selectionMapping[selectedType]) {
      Object.keys(selectionMapping[selectedType]).forEach(function(key) {
        ...
        option.addEventListener('click', function(event) {
          const selectedDisplay = deviceReadDropdown.querySelector('.selected-option');
          if (selectedDisplay) {
            selectedDisplay.setAttribute('data-value', key);
            selectedDisplay.textContent = key;
            localStorage.setItem('smartui_diagselect_device_read', key);
          }
          options.style.display = 'none';
          event.stopPropagation();
        });
  ...
  if (deviceTypeDropdown && deviceReadDropdown) {
    ...
    // Device type dropdown selection (event delegation)
    deviceTypeDropdown.addEventListener('click', function(event) {
      const option = event.target.closest('.dropdown-options [data-value]');
      if (option) {
    ...
    // Device read dropdown selection (event delegation)
