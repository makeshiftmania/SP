...
function populatePageFields(data) {
  ...
  // Populate full address (custom-dropdown version)
  const addressWrapper = document.getElementById("full_address");
  if (addressWrapper) {
    const parts = [];
    ...
    const selected = addressWrapper.querySelector('.selected-option');
    const dropdown = addressWrapper.querySelector('.dropdown-options');

    if (selected) selected.textContent = fullAddress;
    if (dropdown) {
      dropdown.innerHTML = '';
      const item = document.createElement('div');
      item.textContent = fullAddress;
      dropdown.appendChild(item);

      // Dynamically size dropdown to fit full text
      const measure = document.createElement('span');
      measure.style.visibility = 'hidden';
      measure.style.whiteSpace = 'nowrap';
      measure.style.position = 'absolute';
      measure.style.font = getComputedStyle(dropdown).font;
      measure.textContent = fullAddress;
      document.body.appendChild(measure);
      const width = measure.offsetWidth + 20;
...
/* Toggle open/close for custom dropdowns including full_address */
document.addEventListener("click", function (e) {
  ...
  if (clickedDropdown) {
    const options = clickedDropdown.querySelector(".dropdown-options");
    if (options) {
