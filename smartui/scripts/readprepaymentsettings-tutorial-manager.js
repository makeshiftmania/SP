/**
 * Read Prepayment Settings Tutorial Manager
 * 
 * This script manages the tutorial state for the read prepayment settings page.
 * It handles transitions between different tutorial steps by updating
 * the background image and highlighting table headers.
 */

// Tutorial state management
const prepaymentSettingsTutorialState = {
  ...
  updateUI: function() {
    ...
    // Add active class to headers for current step
    step.activeHeaders.forEach(headerClass => {
      const headers = document.querySelectorAll(`.${headerClass}`);
      headers.forEach(header => {
        header.classList.add('tutorial-active');
      });
    });
  },
  
  /**
   * Add CSS styles for tutorial highlighting
   */
  ...
  init: function() {
    ...
    console.log('Read Prepayment Settings Tutorial manager initialized');
  },
  ...
  setupEventListeners: function() {
    ...
    const executeButton = document.getElementById('executeReadPPS');
    if (executeButton) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.body.getAttribute('data-level') === 'bronze' && 
    ...
    prepaymentSettingsTutorialState.init();
  }
}); 